CLASS zcl_quickjs DEFINITION PUBLIC.
  PUBLIC SECTION.
    CLASS-METHODS run
      RAISING
        zcx_wasm.
...
* https://www.npmjs.com/package/@jitl/quickjs-wasmfile-debug-sync
    ...
    DATA(lo_env) = CAST zif_wasm_module( NEW zcl_quickjs_env( ) )->instantiate( ).
    DATA(lo_wasi) = CAST zif_wasm_module( NEW zcl_quickjs_wasi_preview( lo_env->get_memory( ) ) )->instantiate( ).

    GET RUN TIME FIELD DATA(lv_start).
    gi_wasm = zcl_wasm=>create_with_base64(
      iv_base64  = zcl_quickjs_wasm=>get( )
      it_imports = VALUE #(
